...
const parseGLSL = (source) => {
  ...
  lines.forEach((line) => {
    const [, type, name, jsonString] = line.match(
      /uniform (.+?) (.+?);.+\/\/.+ms\((.+?)\)/,
    )
    let json = {}
...
class MagicShader extends RawShaderMaterial {
  constructor(params) {
    ...
    const magicUniforms = parseShaders(
      params.vertexShader,
  ...
  bindUI() {
    ...
    Object.keys(this.magicUniforms).forEach((key) => {
      ...
      const folder = this.gui.addFolder(
        magicJson.name || `🔮 ${magicUniform.type} - ${key}`,
      )
      ...
      if (uniform.value instanceof Color) {
        ...
        Object.keys(uniform.value).forEach((index) => {
          ...
          if (
            magicJson.range &&
            magicJson.range[index] &&
            magicJson.range[index].length === 2
          ) {
  ...
  checkProgram() {
    if (this.program && this.program.program) {
      ...
      this.program.program.__SPECTOR_rebuildProgram = this.rebuildShader.bind(
        this,
      )
